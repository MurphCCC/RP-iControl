...
/*
* This script has been modified to do the following:
* -Scan the current directory and display a thumbnail of all JPG's in the directory
* -Grab filenames for all JPG's in the directory, sort them and put them into a text file one per line.
* -Take this playlist file and send it to feh on the command line to start a fullscreen slideshow with the chosen images.
* Currently a refresh of the page will cause the reload_playlist function to run again cause the script to regenerate the playlist.
*
* Still needs some tweaking but for now this is a simple all in one solution if you need to give users the ability to create/add/edit
* content on a TV with a raspberry pi attached.
*/
...
shell_exec('bash launch_feh.sh >> /dev/null &');
